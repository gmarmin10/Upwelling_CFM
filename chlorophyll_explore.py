...
from numpy import * 
from matplotlib.pyplot import plot, xlim, ylim, show, xlabel, ylabel,subplot, figure, tight_layout,stackplot,title,legend,scatter,xticks, yticks,pcolormesh,clim
from math import *
from math import pi
from time import *
from pylab import *
...
def mac_all_sim(gro,Qn,loop_size,Arr):
    ...
    return Qc_chl_plot,Photo_plot,Bio_plot,cStorage_plot,Essential_plot,nstorage_plot,n_pho,n_bio,n_ess,n_sto,Qc_chl
 
 
neu_Qc_chl_plot,neu_Photo_plot,neu_Bio_plot,neu_cStorage_plot,neu_Essential_plot,neu_nstorage_plot,neu_n_pho,neu_n_bio,neu_n_ess,neu_n_sto,neu_Qc_chl=mac_all_sim(neu_gro,neu_Qn,801,neu_Arr)
el_Qc_chl_plot,el_Photo_plot,el_Bio_plot,el_cStorage_plot,el_Essential_plot,el_nstorage_plot,el_n_pho,el_n_bio,el_n_ess,el_n_sto,el_Qc_chl=mac_all_sim(el_gro,el_Qn,801,el_Arr)
la_Qc_chl_plot,la_Photo_plot,la_Bio_plot,la_cStorage_plot,la_Essential_plot,la_nstorage_plot,la_n_pho,la_n_bio,la_n_ess,la_n_sto,la_Qc_chl=mac_all_sim(la_gro,la_Qn,801,la_Arr)
  
ne_leg=Line2D([], [], color='#DDDDDD',linewidth=3, label='Neutral')
el_leg=Line2D([], [], color='#FFCCCC', linewidth=3,label='El Ni$\~n$o')
la_leg=Line2D([], [], color='#77AADD', linewidth=3, label='La Ni$\~n$a')
han=[ne_leg,el_leg,la_leg]

neu_chlo=neu_Qc_chl*neu_Cbio/55
el_chlo=el_Qc_chl*el_Cbio/55
la_chlo=la_Qc_chl*la_Cbio/55
#now in umol/L#
# 
# neu_chlo=neu_chlo*(893.5*1000)
# el_chlo=el_chlo*(893.5*1000)
# la_chlo=la_chlo*(893.5*1000)
...
plot(x3,la_chlo,marker='p',color='#77AADD')
plot(x1,neu_chlo,marker='o',color='#DDDDDD')
plot(x2,el_chlo,marker='v',color='#FFCCCC')
ylabel('Chlorophyll (\u00B5M)',fontsize=16)
xlabel('Distance from coast (km)',fontsize=16)
xticks(fontsize=12)
yticks(fontsize=12)
title('Modeled Chlorophyll-a',fontsize=18)
legend(handles=han,loc='upper center', bbox_to_anchor=(0.6,1.0),ncol=3,frameon=False,fontsize='large')
sf('mchl',500)

neutral_chl=genfromtxt('neutral_chl.csv',delimiter=',')
lat_neut=neutral_chl[2:108,19]
lon_neut=neutral_chl[2:108,20]
chl_neut=neutral_chl[2:108,21]

el_chl=genfromtxt('el_chl.csv',delimiter=',')
lat_el=el_chl[2:108,18]
lon_el=el_chl[2:108,19]
chl_el=el_chl[2:108,20]

la_chl=genfromtxt('la_nina_chl.csv',delimiter=',')
lat_la=45
lon_la=arange(-127,-123,1)
print(lon_la)
chl_la=(la_chl[45,52:56])
print(chl_la)

chl_neut=chl_neut/(1000*1000*893.5*1e-6)
chl_el=chl_el/(1000*1000*893.5*1e-6)
chl_la=chl_la/(1000*1000*893.5*1e-6)
...
plot(-lon_neut,chl_neut,marker='o',color='#DDDDDD')
plot(-lon_el,chl_el,marker='v',color='#FFCCCC')
plot(-lon_la,chl_la,marker='p',color='#77AADD')
ylabel('Chlorophyll (\u00B5M)',fontsize=16)
xlabel('Longitude',fontsize=16)
xticks(fontsize=12)
yticks(fontsize=12)
title('Sensed Chlorophyll-a',fontsize=18)
legend(handles=han,loc='upper center', bbox_to_anchor=(0.6,1.0),ncol=3,frameon=False,fontsize='large')
sf('schl',500)
